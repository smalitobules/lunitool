...
# Thorium-Installations-Skript
cat > /usr/local/bin/install-thorium.sh <<'EOF'
#!/bin/bash

LOG_FILE="/var/log/thorium-install.log"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "=== Thorium Installation Start $(date) ==="

if [ -f "/var/cache/thorium/thorium.deb" ]; then
    echo "Installiere Thorium-Browser..."
    apt install -y --fix-broken /var/cache/thorium/thorium.deb
    
    if [ -f /usr/bin/thorium-browser ]; then
        echo "Thorium-Browser erfolgreich installiert!"
...
# Service selbst deaktivieren
systemctl disable thorium-install.service

echo "=== Thorium Installation Ende $(date) ==="
EOF

chmod 755 /usr/local/bin/install-thorium.sh

# systemd-Service erstellen
cat > /etc/systemd/system/thorium-install.service <<EOF
[Unit]
Description=Install Thorium Browser
...
[Service]
Type=oneshot
ExecStart=/usr/local/bin/install-thorium.sh
RemainAfterExit=yes
TimeoutSec=60
...
# Service aktivieren
systemctl enable thorium-install.service
